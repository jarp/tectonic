class LocationService
    ...
    def self.lookup(state)
      puts  "[location srv] fetching target location from google"
    ...
    def self.distance(from,state)
        endpoint = "https://maps.googleapis.com/maps/api/distancematrix/json?units=imperial&origins=#{from.gsub('|',',')}&destinations=State of #{state}&key=#{ENV['TECTONIC_API']}"
